...
library(tidyverse)
#predictions commented
x <- c(1, 2, 3, 4, 5, 6)
x
typeof(x) #double
class(x) #vector x => numeric
str(x) #1 2 3 4 5 6 => num [1:6] 1 2 3 4 5 6 
...
1 > 0
1 < 0
1 <- 1  # This will cause an error - why? #lhs to assignment
1 == 1
...
*Write your thoughts here:*
factors can store strings as integers. 
...
One is filled by rows and the other by columns. Default filling is by rows.
...
The provided values get recycled. 3D arrays are seen as two subsets of arrays
...
*Write your analysis here:*
Lists can store different types of data but vectors can only store numeric values.
...
Tibbles have columns with headings and at the top it gives the number of rows and columns
...
filter(babynames, name %in% c("Acura", "Lexus", "Yugo")) #names in this vector
...
#placeholder is needed because f(x,y) is different to f(y,x)
# x |> f(y, _)
```
...
#groups = drop means it will drop that column from your table
```
